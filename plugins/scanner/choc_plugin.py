...
class ChoCHPlugin(BasePlugin):
    ...
    def process(
        ...
        if latest_event.break_index != latest_index:
            return []
    ...
    def build_notes(
        ...
        lines = [
            ...
            f"Trend after break: {state.trend}",
            "",
            "Break",
            f"Swing: {swing.label} {swing.kind}",
            f"Swing Price: {swing.price:.8f}",
            f"Close Price: {event.break_price:.8f}",
            f"Break Distance: {metrics['break_distance']:.8f}",
            "",
            "Filters",
            f"Body: {metrics['body_ratio'] * 100:.2f}%",
            f"Volume: {metrics['volume_ratio']:.2f}x average",
        ]

        if metrics["atr"] is not None:
            lines.extend(
                [
                    f"ATR(14): {metrics['atr']:.8f}",
                    (
                        "Minimum ATR Break: "
                        f"{metrics['min_break_distance']:.8f}"
                    ),
                ]
            )
